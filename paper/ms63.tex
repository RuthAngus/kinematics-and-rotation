...
\citep{curtis2019}.
\racomment{
    In addition, new rotation period measurements for low-mass stars in
...
\\racomment{
    seen in these clusters \citep{spada2019}.
}
...
\racomment{
    (if any at all)
}, because its velocity depends on its current
...
\racomment{
    There is a long history of using kinematic ages to explore the
...
Although {\it vertical} velocity, \vz, is \racomment{
    a well-established
}
age proxy, it can only be calculated with full 6-dimensional position and
...
\racomment{
    The calibration of this relation is described in Curtis \etal\ (2020,
...
\racomment{
    Ground-based rotation period measurements of early M dwarfs
...
(around 3,000 stars), stars fainter than 16th magnitude \racomment{
    in \gaia\ G-band
}
(200 stars), stars with absolute \vb\ uncertainties greater than 1
...
\racomment{
    These rapid rotators are probably either very young stars, or
...
These gyrochrones are \racomment{
    plotted for the same ages described above.
}
...
old ages \racomment{
    compared to higher-mass stars,
}
but their cores are more
tightly coupled to their envelopes, allowing angular momentum transport
between the two layers.
Angular momentum surfaces \racomment{
    from the core
}
and prevents the stellar
...
previously \racomment{for M dwarfs} \citep[\eg][]{west2008, reiners2009,
...
\racomment{
    these phenomena are related, \ie\
