...
\racomment{ Kinematic ages have been used to explore the evolution of cool
dwarfs many times in the past, \citep[\eg][]{reid1995, gizis2000, west2004,
...
gyrochronology model is over-plotted for comparison.
We used this model as an example, however all empirical gyrochronology models
are similar in essence to the \citet{angus2019} relation and none is able to
...
Alternatively, we could have chosen to compare our data with one of the many
other available empirical gyrochronology models.
...
\ref{fig:vplot} because they have a universal power-law index, $n$ (around
...
reproduce the observed trends seen in the velocity dispersion data.
It has a similar period-\teff\ relation to the Praesepe-based
\citet{angus2019} model, because the period-\teff\ relation of Praesepe
roughly follows a line of constant Rossby number.
Consequently, like the \citet{angus2019} model, the \citet{barnes2010}
gyrochrones do not reproduce the data explored here and do not have the
flexibility to capture mass and time-variable rotational evolution.

The lack of flexibility that is baked-in to these empirical gyrochronology
models should not be seen as a failing of gyrochronologists: with a paucity of
calibration data for old stars all one can do is fit simple relations to young
stars and extrapolate to old ages.
...
Angular momentum resurfaces from the core and prevents the stellar envelopes
from spinning-down rapidly, and this effect is strongest for late K-dwarfs
with effective temperatures of $\sim$4000-4500K and masses $\sim$0.5-0.7
M$_\odot$.
...
massive stars.
\racomment{ Although later-type stars are known to remain active for longer,
this result is not necessarily altogether expected.
For example, \citet{matt2015}, found that the upper-envelope of the \kepler\
rotation period distribution corresponds to a single-age population of around
4 Gyr.
Their gyrochronology model coincided with the upper-envelope at 4 Gyrs, and
the natural conclusion was that stars with rotation periods near the
upper-edge of the \mct\ rotation period distribution are all the same age.
However, the velocity dispersion data presented in figure \ref{fig:vplot}
indicate that the upper-envelope is {\it not} a single-age population, and
that later-type stars are in fact older.
}
...
If, for example, the rotation periods of G stars are only detectable when they
are young, G stars with {\it measured periods} should have smaller velocity
dispersions than {\it all} G stars, because they are younger on average.
